  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        process.env.REACT_APP_RIDER_URL + "/drivers-otp-verify",
        {
          otp: formValue.otp,
        }
      );
    ...
    <>
      <div className="  relative">
        ...
        <div className="max-lg:w-11/12 w-1/2 max-lg:left-5 left-1/4 absolute  top-44  max-lg:h-4/5">
          <div>
            <h2 className="text-5xl text-lanternOrange">Welcome to Lantern</h2>
            <h6 className="font-sans mt-5 font-normal">
              Please Enter OTP sent to your phone number to activate your account
            </h6>
          </div>
