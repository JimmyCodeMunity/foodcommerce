  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        process.env.REACT_APP_DRIVER_URL + "/delivery-company-login",
        {
          email: email,
          password: password,
        }
      ...
      if (data.status === "success") {
        ...
        history(`/driveradmin/driverhome/${data.user.id}`);
      } else {
        setLoading(false);
...
export default DriverAdminLogin;
