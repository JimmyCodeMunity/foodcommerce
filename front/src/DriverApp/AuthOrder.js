...
const AuthOrder = () => {
  ...
  const authenticateOrder = async () => {
    setLoading(true)
    try {
      const response = await apiRider.post(`/driver/cook-pick-order/${Id}`, { otp });
  ...
  return (
    <div className="h-full ">
      ...
      <Link to="/driver/arrived">
        <h6 className="h-14 flex font-sans items-center  w-full bg-lanternOrange text-white">
          <ChevronLeft />
          Authentication
        </h6>
      </Link>
      <div className="h-2/3 my-auto flex flex-col justify-evenly font-sans">
        {wrong === true ? (
          <img
            ...
            className="w-44 h-48 my-auto mx-auto"
          />
        ) : (
          <img
            src={wrong2}
        ...
        <div className="w-11/12 mx-auto flex justify-evenly h-1/3 flex-col">
          <h6 className="text-center text-lanternOrange text-xl h-full">
            Authenticate
          </h6>
          {error != "" ? (
            ...
            <h6 className="text-center text-xl font-normal">
              Please enter OTP provided by the cook to verify you picked the
              order
            </h6>
          )}
          <TextField
